...
CATEGORY_INCLUDE = {
    "🎪 中心部のイベント情報": [
        ...
        "期間限定カフェ", "ラテアート",
        # 飲食以外の、中心部（大通・すすきの・駅前等）で行われる幅広いイベント
        "花火大会", "花火", "サマーフェスティバル", "フェスティバル", "フェス",
        "夏まつり", "夏祭り", "秋まつり", "秋祭り", "北海道マラソン", "パレード",
        "イルミネーション", "ホワイトイルミネーション", "クリスマスマーケット",
        "北海道神宮祭", "よさこいソーラン", "YOSAKOI",
    ],
    "🖼️ 展示会": [
        # アニメ関連の展示会（従来通り）
...
CATEGORY_EXCLUDE = {
    "🎪 中心部のイベント情報": ["レストラン", "居酒屋"],  # 普通の飲食店の宣伝は除外（"カフェ"は単体では除外しない。"カフェイベント"等の
...
- 🎪 中心部のイベント情報: 中央区（大通・すすきの・駅前等）で行われる幅広いイベント。
  グルメフェス、フードフェス、マルシェ、物産展、食べ歩き・食べ比べイベント、スイーツイベント、
  ラーメン/肉/餃子フェス、ビアガーデン、日本酒・ワイン・クラフトビール系イベントに加え、
  花火大会、夏祭り・秋祭り、パレード、イルミネーション、よさこいソーラン等、飲食に限らない
  中心部の季節イベント・お祭り全般も含める。普通のレストラン・カフェ・居酒屋の宣伝は含めない。
- 🖼️ 展示会: アニメの原画展、企画展、特別展、コラボカフェ、複製原画展、物販イベントに加え、
  アニメに限らない一般的な展示会・企画展・特別展・写真展・美術展・個展も含める。
...
{"categories": ["🎪 中心部のイベント情報"], "blurb": "30文字程度の魅力が伝わる紹介文"}
...
def build_html(rows, today: str, new_count: int, page_kind: str = "started") -> str:
    if page_kind == "upcoming":
        ...
        category_order = [c for c in CATEGORY_ORDER if c != "🎬 映画"]
    else:
